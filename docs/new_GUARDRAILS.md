...
- Treat changes that may delete or recreate core infrastructure resources as high risk.
- If a change may cause data loss, downtime, or permission drift, call it out clearly before completion.
...
- If validation or planning cannot be executed because prerequisites are missing, report that fact explicitly instead of guessing.

## Bootstrap Safety

- Treat everything under bootstrap as high-risk infrastructure.
- Never change bootstrap resources casually.
- Never assume bootstrap changes are handled by the normal CI/CD pipeline.
- Always note when a bootstrap change requires manual execution.
